...
func GetFilebeatConfig() ([]byte, error) {
	...
	if err := filebeatConfigTemplate.Execute(buf, map[string]interface{}{}); err != nil {
		return nil, err
	}
...
func init() {
	var err error
	if filebeatConfigTemplate, err = template.New(FILEBEAT_CONFIG_FILENAME).Parse(`################### Filebeat Configuration Example #########################
...
output:
  ...
  logstash:
    ...
    tls:
      # List of root certificates for HTTPS server verifications
      certificate_authorities: ["/etc/filebeat/certs/logstash.crt"]

      # Certificate for TLS client authentication
      #certificate: "/etc/filebeat/certs/logstash.crt"

      # Client Certificate Key
      #certificate_key: "/etc/filebeat/certs/logstash.key"
