(function() {
	...
	document.addEventListener('DOMContentLoaded', function () {
		var graphs = document.getElementsByClassName('graph-builder');

		for (var i = 0 ; i < graphs.length ; i++) {
			build_graph(graphs[i], extract_data(graphs[i]));
		}
	});

	function build_graph(node, data) {
	...
	function extract_data(node) {
		return JSON.parse(node.attributes['data-graph'].value);
	}
